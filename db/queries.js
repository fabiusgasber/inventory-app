...
const fetchAllFromTable  = async (table) => {
    ...
    if(!rows.length) throw new Error(`could not find items from table: ${table}`);
    return rows;
    } catch(err){
...
const addMovie = async (formData) => {
    ...
    if(!title || !length || !description || !price || !rating || !quantity || !genre) throw new Error("please provide every required data");
    const associatedActors = Array.isArray(actors) ? actors : [actors];
    const src = "https://placehold.co/600?text=Movie&font=roboto";
    try {
        const { rows } = await pool.query("INSERT INTO movies (title, length, description, price, rating, quantity, src) VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING id", [title, length, description, price, rating, quantity, src]);
        await pool.query("INSERT INTO movie_genres (movieid, genreid) VALUES($1, $2) ON CONFLICT (movieId) DO UPDATE SET movieId=$1, genreId=$2", [rows[0]?.id, genre]);
...
const addGenre = async(formData) => {
    ...
    try{
        const { rows } = await pool.query("INSERT INTO genres (genre, src) VALUES($1, $2) RETURNING id", [title, src]);
        if(!rows || !rows[0] || !rows[0]?.id) throw new Error("there was a problem adding the genre to the database.");
        for(const movieId of associatedMovies){
            await pool.query("INSERT INTO movie_genres (movieId, genreId) VALUES($1, $2) ON CONFLICT (movieId) DO UPDATE SET movieId=$1, genreId=$2", [movieId, rows[0]?.id]);
        }
    } catch(err){
...
const addActor = async(formData) => {
   ...
   if(!firstName || !lastName || !birthdate || !country) throw new Error("please provide every required data");
   const associatedMovies = Array.isArray(movies) ? movies : [movies];
    const src = "https://placehold.co/600?text=Actor&font=roboto";
   try {
        const { rows } = await pool.query("INSERT INTO actors (firstName, lastName, birthdate, country, src) VALUES($1, $2, $3, $4, $5) RETURNING id", [firstName, lastName, birthdate, country, src]);
        if(!rows || !rows[0] || !rows[0]?.id) throw new Error("there was a problem adding the actor to the database.");
        for(const movieId of associatedMovies){
